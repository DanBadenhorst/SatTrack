...
- **Email delivery requires a verified Resend domain.** Until a domain is verified, Resend's sandbox sender only delivers to the account owner's own email; set `RESEND_FROM_EMAIL` (with a verified domain) to reach external recipients. The domain could not be verified in time for submission, so emails will only be sent to the provided test-account user. Emails are sent for configured alerts and when an alert is first configured, so they can be checked on demand by configuring an alert for the test-account user. Emails are also sent when group messages are posted, but again only when external users send to the domain user / test account for now — until the domain can be registered in Resend.
