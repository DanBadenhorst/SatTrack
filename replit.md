...
- **Deployment target:** Vercel (auto-deploys from GitHub `DanBadenhorst/SatTrack` on push). See `artifacts/satellite-tracker/DEPLOYMENT.md` for the full setup (Root Directory = `artifacts/satellite-tracker`, env vars, Supabase callback URLs).
...
- **Alert pipeline** — `POST /api/alerts` is a cron-safe endpoint that sends a **daily pass digest**. For each active subscription it sends, at 13:00 in the subscriber's local time zone on a selected weekday, an email listing the satellite's upcoming passes (over the alert's own `look_ahead_days` window, filtered by the alert's min-elevation/pass-type). It dedupes one send per local calendar day via `sent_alerts` (the `pass_start_utc` column is repurposed as a per-day marker = epoch of local-date midnight). **The cron must run at least hourly** so 13:00 can be matched across time zones. Protect it with `ALERT_CRON_SECRET` or `CRON_SECRET` env var in production (the route accepts either; if neither is set it fails closed with 500 in production). On Vercel, `artifacts/satellite-tracker/vercel.json` registers a **daily** cron at **11:00 UTC** hitting `GET /api/alerts` (Vercel Cron is GET-only and auto-injects `Authorization: Bearer <CRON_SECRET>`; the GET handler delegates to the POST digest logic when a Bearer token is present, else returns health status). **All users are in South Africa (SAST = UTC+2), so 11:00 UTC = 13:00 local — one daily run covers everyone and runs on the free Hobby plan.** If users in other time zones are ever added, the cron must go hourly (`0 * * * *`) to catch each zone's 13:00, which requires Vercel Pro. The immediate confirmation email is sent by `POST /api/alerts/subscriptions` right after the subscription is created (best-effort; failure is logged, not fatal). Pass times in both emails are rendered in the subscriber's stored IANA `timezone` (passed to `sendPassDigest` as `timeZone`) so they match the Passes-page UI (which renders in the browser's local zone). If `timezone` is null the email falls back to the server zone (UTC on Vercel). On Vercel the scheduling is handled by `vercel.json` (above). For any other host, wire an external hourly scheduler hitting the URL with the `Authorization: Bearer $ALERT_CRON_SECRET` header. Dedupe is by INSERT into `sent_alerts` (one row per subscription per local day) — processed alerts are **marked sent, never deleted**; a failed send deletes the claim row so the next hourly run retries the same day.
...
- `NEXT_PUBLIC_SUPABASE_URL` secret includes `/rest/v1/` — the app strips this at runtime. Best to update the secret to just `https://<your-project-ref>.supabase.co`.
