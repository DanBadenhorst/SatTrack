...
- **Weather visibility hint** — `lib/weather.ts` fetches hourly cloud cover from Open-Meteo (free, no API key) for the observing location; `POST /api/passes` makes ONE Open-Meteo call per fetch and enriches each pass with `cloudCover` (% at the pass-start hour). The results UI shows a sky badge (☀️ clear <25%, ⛅ partly 25–60%, ☁️ overcast >60%); passes beyond the ~16-day forecast horizon show no badge. Best-effort — weather failure is logged and never blocks pass predictions. Cached 30 min in-process.
